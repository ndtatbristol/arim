...
_, _, time_to_surface = find_probe_loc_from_frontwall(
    frame_frontwall_meas, couplant, **conf["frontwall_meas"]
)
...
with arim.helpers.timeit("Filtering scanlines"):
    frame_filtered = frame.apply_filter(
        arim.signal.Hilbert()
        + arim.signal.ButterworthBandpass(5, 3e6, 5.5e6, frame.time)
    )
...
for i, view in enumerate(views.values()):
    with arim.helpers.timeit("TFM {}".format(view.name), logger=logger):
        tfms[view.name] = arim.im.tfm.tfm_for_view(
            frame_filtered, grid, view, fillvalue=conf["tfm.fillvalue"]
        )
...
if conf["plot.tfm"]:
    ...
    for i, (viewname, view) in enumerate(views.items()):
        ...
        if conf["plot.show_rays"]:
            ...
            aplt.draw_rays_on_click(
                grid, view.tx_path.rays, element_index, ax, linestyle_tx
            )
            aplt.draw_rays_on_click(
                grid, view.rx_path.rays, element_index, ax, linestyle_rx
            )
