...
# Detect frontwall:
_, _, time_to_surface = find_probe_loc_from_frontwall(
    frame, couplant, **conf["registration"]
)
...
model_options = dict(
    frequency=frame.probe.frequency,
...
for pathname, path in paths.items():
    ...
    with arim.helpers.timeit("Ray weights for {}".format(pathname), logger=logger):
        if path in all_tx_paths:
            ray_weights, ray_weights_debug = bim.tx_ray_weights(
            ...
            debug_data["direct"] = dict(
                ray_weights=ray_weights_debug, sensitivity=sensitivity
            )
        if path in all_rx_paths:
            ray_weights, ray_weights_debug = bim.rx_ray_weights(
                path, ray_geometry, **model_options
            )
    ...
    if conf.get("debug", False):
        ...
        for tx_or_rx, d in debug_data.items():
            if conf.get("debug.transrefl", False):
                i = 0
                aplt.plot_oxz(
                    np.abs(d["ray_weights"]["transrefl"][i]),
                    grid,
            ...
            if conf.get("debug.directivity", False):
                i = 0
                aplt.plot_oxz(
                    np.abs(d["ray_weights"]["directivity"][i]),
                    grid,
...
if conf["plot.sensitivity_view"]:
    ...
    for viewname, view in views.items():
        view_sensitivity_dict[viewname] = arim.model.sensitivity_conjugate_for_view(
            tx_sensitivity_dict[view.tx_path], rx_sensitivity_dict[view.rx_path]
        )

    ref_db = max(np.nanmax(np.abs(v)) for v in view_sensitivity_dict.values())
    aplt.plot_oxz_many(
        view_sensitivity_dict.values(),
        grid,
        7,
        3,
        title_list=view_sensitivity_dict.keys(),
        suptitle="Sensitivity",
        scale="db",
        ref_db=ref_db,
...
for viewname, view in views.items():
    amps_tx = arim.im.SensitivityConjugateAmplitudes(
        frame,
        grid,
        tx_ray_weights_dict[view.tx_path],
    ...
    tfm = arim.im.SingleViewTFM(
        frame, grid, view, amplitudes_tx=amps_tx, amplitudes_rx=amps_rx
    )
    tfms.append(tfm)
