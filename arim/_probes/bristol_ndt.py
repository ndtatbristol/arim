...
def _make_ima_25_MHz_64_1d():
    ...
    dimensions = g.Points.from_xyz(np.full(numelements, 0.35e-3),
                             np.full(numelements, 15e-3),
                             np.full(numelements, 0.))
    dead_elements = np.full(numelements, False, dtype=np.bool)
...
def _make_sonaxis_150_MHz_110_1d():
    ...
    dimensions = g.Points.from_xyz(np.full(numelements, 0.14e-3),
                             np.full(numelements, 3.0e-3),
                             np.full(numelements, 0.))
    dead_elements = np.full(numelements, False, dtype=np.bool)

    probe = core.Probe.make_matrix_probe(
        numx=numelements, pitch_x=0.17e-3, numy=1, pitch_y=np.nan,
        frequency=15e6, shapes=shapes, orientations=orientations,
        dimensions=dimensions, dead_elements=dead_elements,
        metadata=metadata,
    )
    return probe
...
makers.append(ProbeMaker(_make_sonaxis_150_MHz_110_1d,
