...
def A_x_F1(x, xi, xi2, h_nodes, z):
    return (F(xi, xi2, h_nodes, 1 - x ** 2)
            / sqrt(2 - x ** 2) * cos((1 - x ** 2) * z))
...
def A_x(xi, xi2, h_nodes, num_nodes):
    ...
    for i, z in enumerate(z_1):
        ...
        if i < 2:  # two first iterations, coefficients a0 and a1
            int_F1_real = scipy.LowLevelCallable(A_x_F1_real.ctypes,
                                                 data_ptr)
            int_F1_imag = scipy.LowLevelCallable(A_x_F1_imag.ctypes,
                                                 data_ptr)
            int_F2_real = scipy.LowLevelCallable(A_x_F2_real.ctypes,
                                                 data_ptr)
            int_F2_imag = scipy.LowLevelCallable(A_x_F2_imag.ctypes,
                                                 data_ptr)
            I_12[i + num_nodes - 1] = (
                4j * (si.quad(int_F1_real, 0, 1, **quad_args)[0]
                      + 1j * si.quad(int_F1_imag, 0, 1, **quad_args)[0])
                + 4 * (si.quad(int_F2_real, 0, 50, **quad_args)[0]
                       + 1j * si.quad(int_F2_imag, 0, 50, **quad_args)[0])
            )
        else:
            int_F_real = scipy.LowLevelCallable(A_x_F_real.ctypes,
                                                data_ptr)
            int_F_imag = scipy.LowLevelCallable(A_x_F_imag.ctypes,
                                                data_ptr)

            I_12[i + num_nodes - 1] = 4j * (si.quad(int_F_real, 0, 70, **quad_args)[0]
                                            + 1j *
                                            si.quad(int_F_imag, 0, 70, **quad_args)[
                                                0])
    I_12[:num_nodes - 1] = I_12[:num_nodes - 1:-1]

    v_ind = np.arange(num_nodes)
    m_ind = np.full((num_nodes, num_nodes), num_nodes - 1) + v_ind[:, np.newaxis] - v_ind
    return I_12[m_ind]


@numba.njit(cache=True)
def A_z_F1(x, xi, xi2, h_nodes, z):
    return (F(xi, xi2, h_nodes, xi - x ** 2)
            / sqrt(2 * xi - x ** 2) * cos((xi - x ** 2) * z))
...
def A_z(xi, xi2, h_nodes, num_nodes):
    ...
    for i, z in enumerate(z_1):
        data[3] = z
        if i < 2:  # two first iterations, coefficients a0 and a1
            int_F1_real = scipy.LowLevelCallable(A_z_F1_real.ctypes,
                                                 data_ptr)
            int_F1_imag = scipy.LowLevelCallable(A_z_F1_imag.ctypes,
                                                 data_ptr)
            int_F2_real = scipy.LowLevelCallable(A_z_F2_real.ctypes,
                                                 data_ptr)
            int_F2_imag = scipy.LowLevelCallable(A_z_F2_imag.ctypes,
                                                 data_ptr)

            I_12[i + num_nodes - 1] = (
                4j * (si.quad(int_F1_real, 0, sqrt(xi), **quad_args)[0]
                      + 1j * si.quad(int_F1_imag, 0, sqrt(xi), **quad_args)[0])
                + 4 * (si.quad(int_F2_real, 0, 50, **quad_args)[0]
                       + 1j * si.quad(int_F2_imag, 0, 50, **quad_args)[0])
            )
        else:
            int_F_real = scipy.LowLevelCallable(A_z_F_real.ctypes,
                                                data_ptr)
            int_F_imag = scipy.LowLevelCallable(A_z_F_imag.ctypes,
                                                data_ptr)
...
def crack_2d_scat_kernel(phi_in, phi_out_array, vel_L, vel_T, density, frequency,
    ...
    for j, phi_out in enumerate(phi_out_array):
        ...
        c_L = (basis_function(xi1 * h_nodes * ev[0])
               * exp(-1j * xi1 * ev[0] * x_nodes))
        c_T = (basis_function(xi2 * h_nodes * ev[0])
               * exp(-1j * xi2 * ev[0] * x_nodes))
        ...
        if use_incident_L:
            ...
            S_LL[j] = (
                1 / 4 * sqrt(2 / pi) * exp(-1j * pi / 4) * xi1 ** (5 / 2) *
                (
                    lame_lambda / (density * omega ** 2) * (np.dot(v_L, nv))
                    + 2 * lame_mu / (density * omega ** 2)
                    * np.dot(v_L, ev) * np.dot(ev, nv)
                ) / sqrt(lambda_L)
            )
            S_LT[j] = (
                1 / 4 * sqrt(2 / pi) * exp(-1j * pi / 4) * xi2 ** (5 / 2) * lame_mu
                / (density * omega ** 2) *
                (
                    np.dot(v_T, tv) * np.dot(ev, nv)
                    + np.dot(v_T, ev) * np.dot(tv, nv)
                ) / sqrt(lambda_T)
            )
        if use_incident_T:
...
def crack_2d_scat_matrix(phi_in_vect, phi_out_array, vel_L, vel_T, density, frequency,
                         use_incident_L, use_incident_T, x_nodes, h_nodes, A_x, A_z,
                         S_LL, S_LT, S_TL, S_TT):
    """
    ...
    for i in range(phi_in_vect.shape[0]):
        crack_2d_scat_kernel(phi_in_vect[i], phi_out_array[:, i], vel_L, vel_T, density,
                             frequency,
                             use_incident_L, use_incident_T, x_nodes, h_nodes, A_x,
                             A_z,
                             S_LL[:, i], S_LT[:, i], S_TL[:, i], S_TT[:, i])
    return S_LL, S_LT, S_TL, S_TT


@numba.jit(nopython=True, nogil=True, cache=True, parallel=False)
def crack_2d_scat_general(phi_in_array, phi_out_array, vel_L, vel_T, density, frequency,
                          use_incident_L, use_incident_T, x_nodes, h_nodes, A_x, A_z,
                          S_LL, S_LT, S_TL, S_TT):
    """
    ...
    for i in range(phi_in_array.shape[0]):
        for j in range(phi_out_array.shape[1]):
            ...
            crack_2d_scat_kernel(phi_in_array[i, j], phi_out_array[i, j:j + 1],
                                 vel_L, vel_T, density,
                                 frequency,
                                 use_incident_L, use_incident_T, x_nodes, h_nodes,
                                 A_x, A_z,
                                 S_LL[i, j:j + 1], S_LT[i, j:j + 1],
                                 S_TL[i, j:j + 1], S_TT[i, j:j + 1])
    return S_LL, S_LT, S_TL, S_TT
