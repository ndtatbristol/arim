...
__all__ = ['detect_surface_from_extrema', 'move_probe_over_flat_surface']
...
def move_probe_over_flat_surface(frame, distance_to_surface, full_output=False):
    ...
    **Warning:** this function modifies the points1 (you might want to make a copy before the call).
    ...
    Notes
    -----
    For *linear points1*: the points1 must be in the PCS, with all elements on the axis Ox. The array will be transformed
    via a rotation of the angle ``theta`` around the axis Oy and a translation, such as the point O(0,0,0) is
