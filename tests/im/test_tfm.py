...
def test_angle_limiter_contact():
    ...
    amplitudes = arim.im.tfm.angle_limit_in_contact(
        grid, probe, np.radians(30), window="rectangular"
    )
    ...
    with pytest.raises(NotImplementedError):
        amplitudes = arim.im.tfm.angle_limit_in_contact(
            grid, probe, np.radians(30), window="fancywindow"
        )
...
def test_angle_limiter_view():
    ...
    probe_interface = arim.Interface(
        *probe.to_oriented_points(), are_normals_on_out_rays_side=True
    )

    backwall = arim.geometry.points_1d_wall_z(
        -10e-3, 10e-3, 15e-3, 200, name="Backwall", is_block_above=False
    )
    backwall_interface = arim.Interface(
        *backwall, are_normals_on_inc_rays_side=True, are_normals_on_out_rays_side=True
    )
...
def test_multiview_tfm(use_real_grid):
    ...
    backwall = arim.geometry.points_1d_wall_z(
        -1e-3, 1e-3, 10e-3, 200, is_block_above=False
    )
