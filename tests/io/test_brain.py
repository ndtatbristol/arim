...
def test_load_expdata(expdata):
    ...
    assert np.allclose(frame.scanlines[0, :4], [0.05468750, 0.05468750, 0.05468750, 0.04687500])  # first scanline
    assert np.allclose(frame.scanlines[-1, :4], [0.07031250, 0.06250000, 0.06250000, 0.06250000])  # last scanline
    ...
    assert frame.metadata['capture_method'] == CaptureMethod.hmc
    assert frame.probe.metadata['probe_type'] == 'linear'
